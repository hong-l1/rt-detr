...
class GatedMultiheadAttention(nn.Module):
    """Multi-head attention with gating mechanism."""
    ...
    def forward(self, query, key, value, attn_mask=None, key_padding_mask=None):
        """Forward pass for GatedMultiheadAttention.

        Args:
            query (torch.Tensor): Query tensor with shape [L, N, E].
            key (torch.Tensor): Key tensor with shape [S, N, E].
            value (torch.Tensor): Value tensor with shape [S, N, E].
            attn_mask (torch.Tensor, optional): Attention mask.
            key_padding_mask (torch.Tensor, optional): Key padding mask.

        Returns:
            (torch.Tensor): Output tensor with shape [L, N, E].
        ...
        if key_padding_mask is not None:
            # Assumes key_padding_mask is (N, S)
        ...
        y = attn @ v
        y = y.permute(2, 0, 1, 3).contiguous().view(tgt_len, bsz, embed_dim)

        # Gate mechanism: gate = sigmoid(X @ W_gate)
        # Using query as X
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        # Self attention
        self.self_attn = GatedMultiheadAttention(d_model, n_heads, dropout=dropout)
        self.dropout1 = nn.Dropout(dropout)
